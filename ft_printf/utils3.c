...
{
	if (flag != 0)
		f->nbr_precision = f_atoi(&stamp[*i]);
	else
		f->nbr_width = f_atoi(&stamp[*i]);
...
void	handle_width_str(t_struct *f, int i, int *leng)
{
	...
	if (!f_strncmp(f->str, "(null)", 6) && f->nbr_precision < 6 && f->dot == 1)
		len = -6;
	else
	{
		len = f->nbr_precision - f_strlen(f->str);
		if (len < 0 && f->dot == 1)
			len = 0;
	}
	if (f->minus == 0)
...
void	handle_width_nbr(t_struct *f, int i, int *leng)
{
	...
	if (f_strlen(f->str) > f->nbr_precision)
		len = f->nbr_width - f_strlen(f->str)
...
int	f_strncmp(char *s1, char *s2, unsigned int n)
{
	unsigned int	i;
